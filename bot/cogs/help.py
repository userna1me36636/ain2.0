...
class Help(commands.Cog):
    ...
    async def help_command(self, ctx: commands.Context, *, query: str | None = None) -> None:
        ...
        if query and query.title() in self.catalog:
            await ctx.reply(embed=self.category_embed(query.title()), mention_author=False)
            return
        await ctx.reply(embed=self.overview_embed(prefix), view=HelpView(self), mention_author=False)
